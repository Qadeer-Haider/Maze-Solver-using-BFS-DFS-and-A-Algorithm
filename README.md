...
- `Maze.txt`, `Maze4.txt`, `Maze5.txt`: Example maze configurations for testing BFS, DFS, and A* algorithms respectively file.
...
In this README structure, placeholders like `images/original_maze.png`, `images/bfs_path.png`, `images/dfs_path.png`, and `images/a_star_path.png` should be replaced with the actual paths to your image files. Make sure to capture screenshots of the visualizations generated by your program and save them in the appropriate directory (`images/` in this case).
...
## Files
- **`Maze.txt`** and **`Maze4.txt`**: Used for BFS and DFS.
- **`Maze2.txt`**, **`Maze3.txt`**, and **`Maze5.txt`**: Used for A*.

## Note
- Ensure proper commenting and readability in the code.
- Plagiarism and any form of academic dishonesty are strictly prohibited and will result in disciplinary action.
...
This project involves implementing a maze solver using three different algorithms: Breadth-First Search (BFS), Depth-First Search (DFS), and A* Search. The maze is represented as a 2D array, where:

- `0` represents open spaces
- `1` represents walls
- `2` represents short walls (only relevant for the A* algorithm)

The starting point is indicated by `S`, and the goal point by `G`.

### Objectives

1. **Read the Maze from a Text File**: Implement a function to read the maze configuration from a text file and convert it into a 2D array.
2. **Implement BFS and DFS**: Write functions to find a path from the start to the goal using BFS and DFS algorithms.
3. **Implement A* Search**: Implement the A* search algorithm to find the most optimal path, taking into account different wall types.
4. **Visualize the Maze**: Create a graphical representation of the maze using the Pygame library, showing the path found by each algorithm.
...
- `MazeSolver.ipynb`: Main script containing all functions and the logic to run the maze solver.
- `Maze.txt`, `Maze4.txt`, `Maze5.txt`: Example maze configurations for testing BFS, DFS, and A* algorithms respectively.

4. Follow the on-screen instructions to choose an algorithm and visualize the path.
